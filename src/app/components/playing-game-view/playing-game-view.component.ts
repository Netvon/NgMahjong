...
import { PlayingBoard, PlayingTile, SpriteSheet, PostMatch } from 'app/models'
...
export class PlayingGameViewComponent implements OnInit {
	...
	loadGroupedBoard(): Array<TileViewModel[]> {
		...
		if ( this.gameBoard ) {
			let mapped = this.gameBoard.tiles.map(a => {
	...
	tilesMatchable(tile1: PlayingTile, tile2: PlayingTile) {
		...
		return ((tile1.tile.suit === tile2.tile.suit) &&
			((tile1.tile.matchesWholeSuit && tile2.tile.matchesWholeSuit) || (tile1.tile.name === tile2.tile.name)))
	}
